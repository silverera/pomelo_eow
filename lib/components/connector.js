...
var handleMessage = function (self, session, msg) {
    ...
    if (msg.id) {
        ...
        var mismatchReqCnt = session.get('mismatchReqCnt');

        if(msg.id < expect) {

            mismatchReqCnt++;
            session.set('mismatchReqCnt', mismatchReqCnt);
    ...
    self.server.globalHandle(msg, session.toFrontendSession(), function (err, resp, opts) {
        ...
        if (__pushMsgs) {
            for (var id in __pushMsgs) {
                ...
                self.send(null, pushMsg.route, pushMsg.resp, [session.id], null, nullCb);
            }
        }
        ...
        self.send(msg.id, msg.route, resp, [session.id], opts, nullCb);
    });
};
